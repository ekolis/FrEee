...
public abstract class Vehicle : INamed, IConstructable, IVehicle, ICombatant, IFoggable
{
    ...
    /// Unscanned vehicles are not considered destroyed by having destroyed components, though, since their components are unknown.
    /// </summary>
    public bool IsDestroyed
    {
        get
        {
            ...
            else if (Empire.Current is null || CheckVisibility(Empire.Current) >= Visibility.Scanned)
            {
                return Components.All(c => c.IsDestroyed || c.HasAbility("Armor"));
            }
            else
            {
                return false;
