...
namespace FrEee.Extensions
{
	public static class CommonExtensions
	{
		...
		public static Delegate BuildDelegate(this MethodInfo method, params object[] missingParamValues)
		{
			...
			var delegateType = method.ReturnType == typeof(void) ? parmTypes.MakeActionType() : parmTypes.MakeFuncType(method.ReturnType);
		...
		{
			var vals = cmd.GetType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).Where(f => !f.GetCustomAttributes(true).OfType<DoNotSerializeAttribute>().Any() && f.GetGetMethod(true) != null && f.GetSetMethod(true) != null).Select(prop => new { prop.Name, Value = prop.GetValue(cmd, new object[0]) });
		...
		public static int TakeShieldDamage(this IDamageable d, Hit hit, int damage, PRNG dice = null)
		{
			...
			var normalShieldPiercing = dt.NormalShieldPiercing.Evaluate(hit);
			var phasedShieldPiercing = dt.PhasedShieldPiercing.Evaluate(hit);
			var normalSDF = dt.NormalShieldDamage.Evaluate(hit).Percent();
			var phasedSDF = dt.PhasedShieldDamage.Evaluate(hit).Percent();
		...
		private static void LogUnitTransferFailed(IDesign design, ICargoContainer src, ICargoContainer dest, int actualTonnage, int desiredTonnage, Empire emp)
		{
