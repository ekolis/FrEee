...
using FGame = FrEee.Game.Objects.Game;

namespace FrEee.Game.Processes;
...
{
	public TurnProcessor(FGame game)
	{
		Game = game;
	}
		...
		if (Game.Empires.Any(e => e.AI != null && (e.EnabledMinisters?.SelectMany(kvp => kvp.Value)?.Any() ?? false)))
		{
			...
			foreach (var i in Game.Empires.Where(e => e.AI != null && (e.EnabledMinisters?.SelectMany(kvp => kvp.Value)?.Any() ?? false)).Select(e => Game.Empires.IndexOf(e)).ToArray())
			{
				try
				{
					...
					Game.Redact();
					Game.CurrentEmpire.AI.Act(Game.CurrentEmpire, Galaxy, Game.CurrentEmpire.EnabledMinisters);
	...
	private void ProcessResourceValueChange(Planet p)
	{
		...
		{
			bool wasFull = p.ResourceValue[r] == Game.Setup.MaxPlanetValue;
			bool wasEmpty = p.ResourceValue[r] == Game.Setup.MinPlanetValue;
			var modifier =
	...
	public void MoveShips()
	{
		...
		foreach (var v in vlist)
		{
			...
			if (Game.CurrentTick == 0d && !v.Orders.OfType<IMovementOrder>().Any())
				v.DealWithMines();
