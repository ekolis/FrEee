...
namespace FrEee.Game.Objects.Space
{
	...
	public class Galaxy : ICommonAbilityObject
	{
		...
		/// <summary>
		/// The setup that was used to configure this game.
		/// </summary>
		...
		public string CommandFileName
		{
			...
			{
				if (PlayerNumber > 0)
					return GameSetup.GameName + "_" + TurnNumber + "_" + PlayerNumber + FrEeeConstants.PlayerCommandsSaveGameExtension;
				else
		...
		public string GameFileName
		{
			...
			{
				if (PlayerNumber > 0)
					return GameSetup.GameName + "_" + TurnNumber + "_" + PlayerNumber + FrEeeConstants.SaveGameExtension;
				else
					return GameSetup.GameName + "_" + TurnNumber + FrEeeConstants.SaveGameExtension;
			}
		}
		...
		{
			get { return Empires.IndexOf(Empire.Current) + 1; }
		}
		...
		public static void ProcessColonyIncome(Colony c)
		{
			...
			{
				p.ResourceValue[kvp.Key] -= Current.GameSetup.StandardMiningModel.GetDecay(kvp.Value, p.ResourceValue[kvp.Key]);
			}
		}
		...
		public static void ProcessResourceValueChange(Planet p)
		{
			...
			{
				bool wasFull = p.ResourceValue[r] == Current.GameSetup.MaxPlanetValue;
				bool wasEmpty = p.ResourceValue[r] == Current.GameSetup.MinPlanetValue;
				var modifier =
				...
				p.ResourceValue[r] = p.ResourceValue[r].LimitToRange(Current.GameSetup.MinPlanetValue, Current.GameSetup.MaxPlanetValue);
				if (!wasFull && p.ResourceValue[r] == Current.GameSetup.MaxPlanetValue && p.Owner != null)
					p.Owner.RecordLog(p, "{0}'s {1} have been completely replenished. Its value is at the absolute maximum.".F(p, r));
				if (!wasEmpty && p.ResourceValue[r] == Current.GameSetup.MinPlanetValue && p.Owner != null)
		...
		public static IEnumerable<Empire> ProcessTurn(bool safeMode, Status status = null, double desiredProgress = 1d)
		{
			...
			if (Current.Empires.Any(e => !e.IsPlayerEmpire && e.AI != null))
			{
				...
				foreach (var i in Current.Empires.Where(e => !e.IsPlayerEmpire && e.AI != null).Select(e => Current.Empires.IndexOf(e)).ToArray())
				{
					...
					Empire.Current = Current.Empires[i];
					Current.Redact();
					Empire.Current.AI.Act(Empire.Current, Current, Empire.Current.AI.MinisterNames);
					cmds.Add(i, Empire.Current.Commands);
					notes.Add(i, Empire.Current.AINotes);
				}
				LoadFromString(serializedGalaxy);
				foreach (var i in Current.Empires.Where(e => !e.IsPlayerEmpire && e.AI != null).Select(e => Current.Empires.IndexOf(e)).ToArray())
				{
				...
				foreach (var kvp in emp.RemoteMiners)
				{
					...
					foreach (var r in Resource.All)
					{
						...
						if (amount > 0 && adjustedValue[mined][r] == 0)
						{
							...
							adjustedValue[mined][r] = Current.GameSetup.RemoteMiningModel.GetDecay(kvp.Value[r], mined.ResourceValue[r]);
			...
			foreach (var vc in Current.GameSetup.VictoryConditions)
			{
				if (vc is TotalEliminationVictoryCondition || Current.TurnNumber > Current.GameSetup.VictoryDelay)
				{
		...
		/// </summary>
		/// <exception cref="InvalidOperationException">if Empire.Current is not null.</exception>
		public static void SaveAll(Status status = null, double desiredProgress = 1d)
		{
			...
			var progressPerSaveLoad = (desiredProgress - (status == null ? 0d : status.Progress)) / (Current.GameSetup.IsSinglePlayer ? 3 : (Current.Empires.Count + 2));
		...
		{
			return GetEmpireCommandsSavePath(GameSetup.GameName, TurnNumber, Empires.IndexOf(emp) + 1);
		}
		...
		public string GetGameSavePath(Empire emp = null)
		{
			if (emp == null)
				emp = Empire.Current;
			return GetGameSavePath(GameSetup.GameName, TurnNumber, emp == null ? 0 : (Empires.IndexOf(emp) + 1));
		}
		...
		public string Save(bool assignIDs = true)
		{
			...
			string filename;
			if (Empire.Current == null)
				filename = GameSetup.GameName + "_" + TurnNumber + ".gam";
			else
				filename = GameSetup.GameName + "_" + TurnNumber + "_" + (Empires.IndexOf(Empire.Current) + 1).ToString("d4") + ".gam";
		...
		public string SaveCommands()
		{
			...
			var filename = GetEmpireCommandsSavePath(Empire.Current);
		...
		{
			if (Empire.Current == null)
				return GameSetup.GameName;
			return Empire.Current.Name + " - " + Empire.Current.LeaderName + " - " + Stardate;
		}
		...
		private void redactParser_StartObject(object o)
		{
			...
			if (o is IFoggable)
			{
				...
				{
					var id = obj.ID;
					var vis = obj.CheckVisibility(Empire.Current);
					if (vis < Visibility.Fogged)
						obj.Dispose();
					if (vis == Visibility.Fogged && Empire.Current.Memory.ContainsKey(id))
					{
						Empire.Current.Memory[id].CopyToExceptID(obj, IDCopyBehavior.PreserveDestination); // memory sight!
						obj.IsMemory = true;
					}
				...
				else
				{
					...
					if (!Empire.Current.Memory.Values.Contains(obj))
						obj.Dispose();
				}
			}
		...
		private void SerializeCommands(Stream stream)
		{
			...
			Serializer.Serialize(Empire.Current.Commands, stream);
		}
