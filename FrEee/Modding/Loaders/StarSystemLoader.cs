	...
	public class StarSystemLoader : DataFileLoader
	{
		...
		public override IEnumerable<IModObject> Load(Mod mod)
		{
			foreach (var rec in DataFile.Records)
			{
				...
				sst.WarpPointAbilities = The.Mod.StellarAbilityTemplates.FindByName(rec.Get<string>("WP Stellar Abil Type", sst));
				...
				while (true)
				{
					...
					{
						Mod.Errors.Add(new DataParsingException("Could not find \"Obj Position\" field.", The.ModFileName, rec));
					...
					if (sobjtype == "Star" || sobjtype == "Destroyed Star")
					{
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Stellar Abil Type\" field for star.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
							...
							if (template.Abilities == null)
							{
								...
								Mod.Errors.Add(new DataParsingException("Could not find stellar ability type \"" + temp + "\" in StellarAbilityTypes.txt.", The.ModFileName, rec));
							}
						}

						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Size", "Obj Size" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Size\" field for star.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
							...
							else
							{
								...
								{
									Mod.Errors.Add(new DataParsingException("Invalid stellar object size \"" + temp + "\". Must be Tiny, Small, Medium, Large, Huge, or Any.", ex, The.ModFileName, rec));
									continue; // skip this stellar object
								}
							}
						}
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Age\" field for star.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Color\" field for star.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Luminosity\" field for star.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
					...
					else if (sobjtype == "Planet")
					{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Stellar Abil Type", "Obj Stellar Abil Type" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Stellar Abil Type\" field for planet.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Size", "Obj Size" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Size\" field for planet.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Atmosphere\" field for planet.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Composition\" field for planet.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
					...
					else if (sobjtype == "Asteroids")
					{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Stellar Abil Type", "Obj Stellar Abil Type" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Stellar Abil Type\" field for asteroid field.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Size", "Obj Size" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Size\" field for asteroid field.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Atmosphere", "Obj Atmosphere" }, out temp, ref start, null, start))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Atmosphere\" field for asteroid field.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
							template.Atmosphere = temp == "Any" ? null : temp;
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Composition", "Obj Composition" }, out temp, ref start, null, start))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Composition\" field for asteroid field.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
							template.Surface = temp == "Any" ? null : temp;
					...
					else if (sobjtype == "Storm")
					{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Stellar Abil Type", "Obj Stellar Abil Type" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Stellar Abil Type\" field for storm.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
						...
						if (!rec.TryFindFieldValue(new string[] { "Obj " + count + " Size", "Obj Size" }, out temp, ref start, null, start, true))
						{
							Mod.Errors.Add(new DataParsingException("Could not find \"Obj Size\" field for storm.", The.ModFileName, rec));
							continue; // skip this stellar object
						}
						else
						{
					...
					{
						Mod.Errors.Add(new DataParsingException("Invalid stellar object type \"" + sobjtype + "\". Must be Star, Planet, Asteroids, or Storm.", The.ModFileName, rec));
						continue;
					}
					...
					if (pos.StartsWith("Ring "))
					{
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not parse stellar object location \"" + pos + "\". Expected integer after Ring.", The.ModFileName, rec));
							continue;
						}
					...
					else if (pos.StartsWith("Coord ") || pos.StartsWith("Centered Coord "))
					{
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not parse stellar object location \"" + pos + "\". Expected two comma separated integers after Coord.", The.ModFileName, rec));
							continue;
						}
					...
					else if (pos.StartsWith("Same As"))
					{
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not parse stellar object location \"" + pos + "\". Expected integer after Same As.", The.ModFileName, rec));
							continue;
						}
						if (idx > sst.StellarObjectLocations.Count)
						{
							Mod.Errors.Add(new DataParsingException("A \"Same As\" stellar object location can reference only previously defined locations.", The.ModFileName, rec));
							continue;
						}
					...
					else if (pos.StartsWith("Circle Radius "))
					{
						...
						{
							Mod.Errors.Add(new DataParsingException("Could not parse stellar object location \"" + pos + "\". Expected integer after Circle Radius.", The.ModFileName, rec));
							continue;
						}
