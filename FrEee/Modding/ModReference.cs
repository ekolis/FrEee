...
{
	// XXX: Just use this class as a temporary holder for links between IDs and objects; add a [SerializeAsModReference] attribute to replace all the duplicated properties
	/// <summary>
	...
	public class ModReference<T> : IReference<string, T> where T : IModObject
	{
		...
		public static implicit operator ModReference<T>(T t)
		{
			...
			if (t.ModID == null)
			{
				...
				The.Mod.Add(t, The.Mod.Objects.Select(q => q.ModID).ToList());
			}
