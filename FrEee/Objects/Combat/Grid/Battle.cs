			...
			for (var i = 0; i < MaxRounds; i++)
			{
				...
				foreach (var c in turnorder)
				{
					...
					if (c is Seeker s)
					{
						...
						s.DistanceTraveled += Min(GetCombatSpeedThisRound(c), locations[s].DistanceToEightWay(locations[s.Target]));
...
gotosAreVeryEvil:
			...
			EndCombatants = Combatants.Select(c => new { c.ID, Copy = c.CopyAndAssignNewID() }).ToDictionary(q => q.ID, q => q.Copy);
		...
		private void CheckSeekerDetonation(Seeker s, SafeDictionary<ICombatant, IntVector2> locations)
		{
			if (locations[s] == locations[s.Target])
			{
				...
				var wasArmed = s.Target is Seeker || s.Target.Weapons.Any();
				s.Target.TakeDamage(hit, Dice);
				var isArmed = s.Target is Seeker || s.Target.Weapons.Any();
			...
			while (reloads[w] <= 0)
			{
				if (w.BurnSupplies())
				{
					...
					else
					{
						...
						var wasArmed = target is Seeker || target.Weapons.Any();
						if (w.Template.ComponentTemplate.WeaponInfo.IsWarhead || shot.RollAccuracy(Dice))
						{
							...
							var isArmed = target is Seeker || target.Weapons.Any();
