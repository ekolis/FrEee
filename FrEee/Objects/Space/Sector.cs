...
namespace FrEee.Objects.Space
{
	...
	public class Sector : IPromotable, ICargoContainer, ICommonAbilityObject, IOwnable
	{
		...
		public bool AddUnit(IUnit unit)
		{
			...
			var fleet = SpaceObjects.OfType<Fleet>().SelectMany(f => f.SubfleetsWithNonFleetChildren()).Where(
		...
		public override string ToString()
		{
			...
			var objectNameContainsStarName = Regex.IsMatch(largestObject.Name, string.Format(@"\b{0}\b", Regex.Escape(StarSystem.Name ?? "(unexplored)")));
