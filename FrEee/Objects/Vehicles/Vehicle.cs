	...
	public abstract class Vehicle : INamed, IConstructable, IVehicle, ICombatant, IFoggable
	{
		...
		public SafeDictionary<MountedComponentTemplate, IList<int>> Damage
		{
			...
			set
			{
				...
				{
					var component = template.Instantiate(The.Game);
		...
		public IDesign<IVehicle> Design { get; set; }
		...
		public IHull<IVehicle> Hull => Design.Hull;
		...
		public IVehicle LatestVersion
		{
			...
			{
				if (IsObsolescent)
					return Design.LatestVersion.Instantiate(The.Game);
				else
					return this;
			}
		}
		...
		public IEnumerable<IVehicle> NewerVersions
		{
			get
			{
				return The.Galaxy.FindSpaceObjects<IVehicle>().Where(v => Design.UpgradesTo(v.Design));
			}
		}
		...
		public IEnumerable<IVehicle> OlderVersions
		{
			get
			{
				return The.Galaxy.FindSpaceObjects<IVehicle>().Where(v => v.Design.UpgradesTo(Design));
			}
		}
		...
		public virtual void Redact(Empire emp)
		{
			...
			if (visibility < Visibility.Scanned)
			{
				...
				// or maybe we should recognize it since it is identical, to avoid exploity design spam?
				if (Design.CheckVisibility(emp) < Visibility.Scanned)
				{
					// create fake design
					var d = Vehicles.Design.Create(Design.Hull);
