			...
			foreach (var tech in Receiver.UnlockedItems.OfType<Tech>())
			{
				...
				if (giveLevels > hasLevels || giveLevels == hasLevels && giveAmount > hasAmount)
				{
					if (RandomHelper.Next(100d) < The.Mod.Settings.CooperativeResearchBreakthroughChance)
					{
						...
						Giver.Log.Add(Receiver.CreateLogMessage("The " + Receiver + " has achieved a breakthrough in " + tech + " due to our cooperative research treaty.", LogMessageType.Generic));
						Receiver.Log.Add(tech.CreateLogMessage("We have achieved a breakthrough in " + tech + " due to our cooperative research treaty with the " + Giver + ".", LogMessageType.ResearchComplete));
					}
				}
			}
		}
