...
namespace FrEee.Objects.Civilization
{
	...
	public class Empire : INamed, IFoggable, IAbilityObject, IPictorial, IComparable<Empire>, IComparable, IFormulaHost, IReferrable
	{
		public Empire()
		{
			...
			PlayerNotes = new SafeDictionary<IReferrable, string>();
			PrivateNames = new SafeDictionary<INameable, string>();
		...
		public IEnumerable<Planet> ColonizedPlanets
		{
			get
			{
				return The.Galaxy.StarSystemLocations.Select(ssl => ssl.Item).SelectMany(ss => ss.FindSpaceObjects<Planet>(p => p.Owner == this && !p.IsMemoryOfKnownObject()));
			}
		}
		...
		public IEnumerable<ConstructionQueue> ConstructionQueues
		{
			get
			{
				return The.Game.Referrables.OfType<ConstructionQueue>().Where(q => q.Owner == this && q.Container.Sector != null && q.Rate.Any(kvp => kvp.Value > 0));
			}
		}
		...
		{
			get { return The.Galaxy.StarSystemLocations.Select(ssl => ssl.Item).Where(sys => sys.ExploredByEmpires.Contains(this)); }
		}
		...
		public ResourceQuantity GrossDomesticIncome
		{
			...
			{
				// shouldn't change except at turn processing...
				if (grossDomesticIncome == null || Current == null)
				{
					...
					if (this != Current)
					{
						...
						var clauses = GivenTreatyClauses.Flatten().OfType<FreeTradeClause>().Where(c => c.Receiver == Current).GroupBy(c => c.Resource);
		...
		public ResourceQuantity Maintenance
		{
			get
			{
				...
				if (maintenance == null || Current == null)
		...
		public IEnumerable<ISpaceObject> OwnedSpaceObjects
		{
			get
			{
				return The.Galaxy.FindSpaceObjects<ISpaceObject>(sobj => sobj.Owner == this);
			}
		}
		...
		/// </summary>
		public SafeDictionary<IReferrable, string> PlayerNotes { get; private set; }
		...
		/// </summary>
		public SafeDictionary<INameable, string> PrivateNames { get; private set; }
		...
		public ResourceQuantity RawResourceIncome
		{
			...
			{
				// shouldn't change except at turn processing...
				if (rawResourceIncome == null || Current == null)
				{
					rawResourceIncome = new ResourceQuantity();
					foreach (var sobj in The.Galaxy.FindSpaceObjects<IIncomeProducer>().BelongingTo(this))
		...
		public IDictionary<(ISpaceObject Miner, IMineableSpaceObject Target), ResourceQuantity> RemoteMiners
		{
			...
			{
				// shouldn't change except at turn processing...
				if (remoteMiners == null || Current == null)
				{
					remoteMiners = new SafeDictionary<(ISpaceObject, IMineableSpaceObject), ResourceQuantity>(true);
					foreach (var miner in The.Galaxy.FindSpaceObjects<ISpaceObject>().BelongingTo(this))
					{
		...
		public ResourceQuantity RemoteMiningIncome
		{
			...
			{
				// shouldn't change except at turn processing...
				if (remoteMiningIncome == null || Current == null)
		...
		public int? Score
		{
			...
			{
				int? s = null;
				for (var x = The.Game.TurnNumber; x >= 0 && s == null; x--)
				{
					s = GetScoreAtTurn(x);
				}
				return s;
		...
		public ResourceQuantity TradeIncome
		{
			...
			{
				// shouldn't change except at turn processing...
				if (tradeIncome == null || Current == null)
				{
		...
		public bool CheckUnlockStatus(IUnlockable item)
		{
			...
			if (item is Tech && ((Tech)item).IsUnique && !UniqueTechsFound.Any(t => t == ((Tech)item).UniqueTechID))
		...
		public int? ComputeScore(Empire viewer)
		{
			...
			var score = 0;
			score += The.Game.Referrables.OfType<IVehicle>().OwnedBy(this).Sum(v => v.Cost.Sum(kvp => kvp.Value)); // vehicle cost
		...
		public void Redact(Empire emp)
		{
			...
			{
				var original = (IFoggable)The.Game.ReferrableRepository[kvp.Key];
		...
		public void UpdateMemory<T>(T obj) where T : IFoggable, IOwnable, IReferrable
		{
			...
			if (obj.ID > 0)
			{
				...
				if (obj is Fleet)
				{
					foreach (var order in OwnedSpaceObjects.OfType<IMobileSpaceObject>().SelectMany(sobj => sobj.Orders))
					{
		...
		private double? GetResearchQueueDelay(Tech tech, int level)
		{
			...
			return costBefore / (queueSpending * totalRP / 100d);
		}
