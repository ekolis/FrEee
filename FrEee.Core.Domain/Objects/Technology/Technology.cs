...
public class Technology : IModObject, IResearchable
{
	...
	{
		if (Game.Current.GameSetup.TechnologyCost == TechnologyCost.Low)
			return LevelCost * level;
		else if (Game.Current.GameSetup.TechnologyCost == TechnologyCost.Medium)
		{
			if (Math.Abs(level) == 1)
				return LevelCost * level;
			else
		...
		else if (Game.Current.GameSetup.TechnologyCost == TechnologyCost.High)
			return LevelCost * level * level;
		throw new Exception("Invalid technology cost for galaxy: " + Game.Current.GameSetup.TechnologyCost);
	}
	...
	public int GetLevelCost(int level, Empire emp)
	{
		...
		if (Game.Current.GameSetup.TechnologyUniqueness == 0)
			return baseCost;
		var playerRatio = emp.OtherPlayersTechLevels[this]?.Count(x => x >= level) ?? 0;
		var uniquenessFactor = Math.Pow(2, Game.Current.GameSetup.TechnologyUniqueness * playerRatio);
