...
public class ScrapBehavior : IRecycleBehavior
{
    ...
    public IEnumerable<LogMessage> GetErrors(IMobileSpaceObject executor, IRecyclable target)
    {
        ...
        if ((target is Ship || target is Base) && !executor.Sector.SpaceObjects.Any(sobj => sobj.Owner == executor.Owner && sobj.HasSpaceYard()))
        ...
        if (target is IUnit && !executor.Sector.SpaceObjects.Any(sobj => sobj.Owner == executor.Owner && (sobj is Planet || sobj.HasSpaceYard())))
