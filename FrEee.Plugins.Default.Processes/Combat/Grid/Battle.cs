                        ...
                        gotosAreEvil:
                                ...
                                && locations[c].DistanceToEightWay(locations[bestTarget]) > DistancesToTargets[c])
                            {
                                ...
                                var roundsToClose = c.Weapons.Where(w => w.Template.ComponentTemplate.WeaponInfo.IsSeeker).Max(w =>
                                    (int)Ceiling(w.Template.WeaponMaxRange / (double)(w.Template.ComponentTemplate.WeaponInfo as SeekingWeaponInfo).SeekerSpeed));
            ...
            foreach (var c in turnorder.Reverse())
            {
                if (c is ICargoTransferrer cc && cc.AllUnits.Where(q => q.CanInvadeAndPoliceColonies).Any())
                {
                    ...
                    if (dropTarget != null)
                    {
                        ...
                        var groundBattle = Services.Battles.CreateGroundBattle(dropTarget);
